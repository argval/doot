...
- What runs on this VM:
  ...
  - Web UI: `npm run dev:web` → Vite on **`http://localhost:1420`**. It binds IPv6 `localhost`, so `http://127.0.0.1:1420` may fail; use `localhost`. Overlay glass uses a `web-preview` backdrop. Caption chrome: `/?preview=captions`, `/?preview=captions-indic`, `/?preview=listening`, `/?preview=error`, `/?preview=progressive`. Settings: `/?window=settings`. Starting capture in the browser shows a desktop-app error; it does not call Tauri.
