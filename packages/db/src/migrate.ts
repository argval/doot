...
export async function migrateDb(filePath: string = resolveDefaultDbPath(), folder = migrationsFolder): Promise<DootDb> {
...
if (entry && /[\\/]migrate\.[jt]s$/.test(entry) && import.meta.url === pathToFileURL(entry).href) {
